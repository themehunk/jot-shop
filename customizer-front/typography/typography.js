...
( function( $ ) {
	...
	BigStrTypography = {
		...
		_initFonts: function()
		{
			$( '.customize-control-bigstr-font-family select' ).each( function(e) {
			...
			$( '.customize-control-bigstr-font-family select' ).each( BigStrTypography._initFont );
			// Added select2 for all font family & font variant.
			$('.customize-control-bigstr-font-family select, .customize-control-bigstr-font-variant select').selectWoo();

			$('.customize-control-bigstr-font-variant select').on('select2:unselecting', function (e) {
		...
		_initFont: function()
		{
			...
			if ( 'undefined' != typeof weight ) {
				api( link ).bind( BigStrTypography._fontSelectChange );
				BigStrTypography._setFontWeightOptions.apply( api( link ), [ true ] );
			}

			if ( 'undefined' != typeof variant ) {
				api( link ).bind( BigStrTypography._fontSelectChange );
				BigStrTypography._setFontVarianttOptions.apply( api( link ), [ true ] );
			}
		},
		...
		_fontSelectChange: function()
		{
			...
			BigStrTypography._setFontWeightOptions.apply( this, [ false ] );
			
			if ( 'undefined' != typeof variants ) {
				BigStrTypography._setFontVarianttOptions.apply( this, [ false ] );
			}
		},
		...
		_cleanGoogleFonts: function(fontValue)
		{
			...
			if ( 'undefined' != typeof BigstrFontFamilies.google[ googleFontValue ] ) {
		...
		_getWeightObject: function(fontValue)
		{
			...
			} else if ( 'undefined' != typeof BigstrFontFamilies.system[ fontValue ] ) {
				weightObject = BigstrFontFamilies.system[ fontValue ].weights;
			} else if ( 'undefined' != typeof BigstrFontFamilies.google[ fontValue ] ) {
				weightObject = BigstrFontFamilies.google[ fontValue ][0];
				weightObject = Object.keys(weightObject).map(function(k) {
				  return weightObject[k];
				});
			} else if ( 'undefined' != typeof BigstrFontFamilies.custom[ fontValue ] ) {
				weightObject = BigstrFontFamilies.custom[ fontValue ].weights;
			}
		...
		_setFontWeightOptions: function( init )
		{
			...
			var fontValue = BigStrTypography._cleanGoogleFonts(fontValue);
			var weightObject = BigStrTypography._getWeightObject( fontValue );
		...
		_setFontVarianttOptions: function( init )
		{
				...
				variantSavedField   = api.control( variants ).container.find( '.bigstr-font-variant-hidden-value' ),
				weightValue        = '',
				weightOptions       = '',
	...
	$( function() { BigStrTypography.init(); } );
