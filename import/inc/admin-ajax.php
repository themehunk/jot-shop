...
class Themehunk_Library_Ajax{
	...
	public static function import_data_filter($demo_api_uri){
		...
		function import_demo_data() {
			...
			if ( ! current_user_can( 'customize' ) ) {
				wp_send_json_error( __( 'You have not "customize" access to import the Themehunk site library.', 'open-mart' ) );
			}
			...
			$demo_api_uri = isset( $_POST['api_url'] ) ? esc_url( $_POST['api_url'] ) : '';
			 ...
			 if ( ! empty( $demo_api_uri ) ) {
				...
				wp_send_json_error( __( 'Request site API URL is empty. Try again!', 'open-mart' ) );
			 }
 ...
 */
			...
			if ( isset( $xml_url ) ) {
				...
				if ( $xml_path['success'] ) {
					...
					if ( isset( $xml_path['data']['file'] ) ) {
						...
						wp_send_json_error( __( 'There was an error downloading the XML file.', 'open-mart' ) );
					}
				} else {
					wp_send_json_error( $xml_path['data'] );
				}
			} else {
				wp_send_json_error( __( 'Invalid site XML file!', 'open-mart' ) );
			}
		}
		...
		function import_customizer_settings() {
			...
			if ( isset( $customizer_data ) ) {
				...
				wp_send_json_error( __( 'Customizer data is empty!', 'open-mart' ) );
			}
		...
		function import_option_data(){
			 ...
			 if ( isset( $options_data ) ) {
				...
				wp_send_json_success( $options_data );
			 	...
			 	wp_send_json_error( __( 'Site options are empty!', 'open-mart' ) );
			 }
		...
		function import_widgets() {
			...
			if ( isset( $widgets_data ) ) {
				...
				wp_send_json_error( __( 'Widget data is empty!', 'open-mart' ) );
			}
